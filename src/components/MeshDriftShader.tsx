...
export default function MeshDriftShader() {
  ...
  useEffect(() => {
    ...
    // Colours (low -> high): #0B1026, #3D46E8, #B18CFF, #FFD6E7
    ...
    // u_space = vec4(0.11, -0.19, pointer x, pointer y) -> offset.xy, pointer.xy
    gl.uniform4f(uSpaceLoc, 0.11, -0.19, 0.0, 0.0);
    // u_cursor = vec4(presence (0), 2.0, 0.65, 0.46) -> cursor off
    ...
    const render = (now: number) => {
      if (isTabVisible) {
        ...
        // u_scene = vec4(canvas width, canvas height, seconds * -0.73, 4.0) (speed 33/100 -> time scaling)
