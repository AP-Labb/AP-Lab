...
import {
  User, Palette, Bell, Crown, Sun, Moon, Check, Info,
  Volume2, ShieldAlert, ArrowLeft, Trash2, Edit3, Key, Mail, Sparkles, Layers
} from "lucide-react";
...
export default function SettingsPage() {
  ...
  const [activeTab, setActiveTab] = useState<"account" | "customize" | "notifications" | "plan">("customize");

  useEffect(() => {
    if (tabParam === "account" || tabParam === "customize" || tabParam === "notifications" || tabParam === "plan") {
  ...
  const handleTabChange = (tab: "account" | "customize" | "notifications" | "plan") => {
    setActiveTab(tab);
  ...
  const handleBannerColorChange = async (colorHex: string) => {
    ...
    if (updatePreferences) {
      await updatePreferences({ profileBannerColor: colorHex });
    }
  };
      ...
      <div className="flex-1 flex flex-col min-h-screen md:pl-16 relative z-10">
        ...
        <main className="max-w-4xl mx-auto w-full px-4 sm:px-6 py-8 pb-24 space-y-6">
          ...
          <div className="flex items-center gap-2 text-xs text-white/40 font-mono">
            ...
            <span>/</span>
            <span className="text-white/80 font-bold">Settings</span>
          </div>
          ...
          <div className="bg-[#090a12] border border-white/[0.08] rounded-3xl p-6 sm:p-8 space-y-6 shadow-[0_24px_60px_rgba(0,0,0,0.8)]">
            <div>
              ...
              <p className="text-sm font-manrope text-white/40 mt-1">
                Manage all your preferences, appearance, and account details
              </p>
            </div>
              ...
              {[
                ...
                return (
                  <button
                    ...
                    className={cn(
                      "flex items-center gap-2 px-5 py-2.5 rounded-full text-xs font-manrope font-bold transition-all cursor-pointer border whitespace-nowrap",
                      isActive
              ...
              <div className="bg-[#090a12] border border-white/[0.08] rounded-3xl p-6 sm:p-8 space-y-5">
                <div>
                  ...
                  <p className="text-xs text-white/40 font-manrope mt-0.5">
                    Change how your profile card and course pages look and feel.
                  </p>
                </div>
                ...
                <div className="space-y-3 pt-2 border-t border-white/[0.06]">
                  <label className="text-xs font-mono font-bold text-white/50 uppercase tracking-wider block">
                    Select Profile Cover Banner Color
                  </label>
          ...
          {activeTab === "account" && (
            ...
            >
              ...
              <form onSubmit={handleSaveAccountInfo} className="bg-[#090a12] border border-white/[0.08] rounded-3xl p-6 sm:p-8 space-y-6">
                <div className="flex items-center justify-between">
                  <div>
                    <h3 className="font-manrope font-extrabold text-xl text-white">Account Details</h3>
                    <p className="text-xs text-white/40 font-manrope mt-0.5">
                  ...
                  <button
                    type="submit"
                    disabled={saving}
                    className="px-5 py-2 rounded-xl bg-white text-black font-manrope font-bold text-xs hover:bg-neutral-200 transition-colors disabled:opacity-50 cursor-pointer shadow-md"
                  >
                ...
                <div className="space-y-1.5 pt-2 border-t border-white/[0.06]">
                  ...
                  <input
                    ...
                    className="w-full bg-white/[0.04] border border-white/10 rounded-2xl px-4 py-3 text-white text-xs font-manrope focus:outline-none focus:border-white/30 transition-colors"
                  />
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-1.5 relative">
                    ...
                    <button
                      ...
                      className="w-full flex items-center justify-between bg-white/[0.04] border border-white/10 rounded-2xl px-4 py-3 text-white text-xs font-manrope text-left"
                    >
                  ...
                  <div className="space-y-1.5">
                    ...
                    <select
                      ...
                      className="w-full bg-[#0e101a] border border-white/10 rounded-2xl px-4 py-3 text-white text-xs font-manrope focus:outline-none"
                    >
          ...
          {/* TAB 3 & 4 PLACEHOLDERS (Notifications / Plan) */}
          {activeTab === "notifications" && (
            <div className="bg-[#090a12] border border-white/[0.08] rounded-3xl p-8 text-center space-y-3">
              <Bell className="w-10 h-10 text-white/20 mx-auto" />
              <h3 className="font-bold text-lg text-white">Notification Preferences</h3>
              <p className="text-xs text-white/40 max-w-sm mx-auto">
                Daily study reminder alerts and goal notifications are enabled for your account.
              </p>
            </div>
          )}
          ...
          {activeTab === "plan" && (
            <div className="bg-[#090a12] border border-white/[0.08] rounded-3xl p-8 text-center space-y-3">
              ...
              <h3 className="font-bold text-lg text-white">Free AP Scholar Plan</h3>
              <p className="text-xs text-white/40 max-w-sm mx-auto">
                You have full access to all AP Lab courses, mock exams, AI tutors, and practice tools!
              </p>
            </div>
          )}
        </main>
